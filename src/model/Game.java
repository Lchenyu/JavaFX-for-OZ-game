...
public abstract class Game {
    ...
    public Game(String referee, String gameType, String id)
    {
        this.referee = referee;
        this.createGameType(gameType);
        this.gameID = id;
...
//		while((s = br.readLine() ) != null ){
        ...
        for(int i = 0; i < flgType.length; i++)
        {
            if(!flgType[i])
            {
                if( i == 0)
                {
                   overwriteGameId("scount.oid",scount);
                   return "sg" + scount;
                }else if(i == 1)
                {
                   overwriteGameId("rcount.oid",rcount);
                   return "rg" + rcount;
                }
                else if(i == 2)
                {
                   overwriteGameId("ccount.oid",ccount);
                   return "cg" + ccount;
                }
            }
        }
        ...
        if (id == null)
        	System.out.println("aaaaaaaaargghhh!!!!");
